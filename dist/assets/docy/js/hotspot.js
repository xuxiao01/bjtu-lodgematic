...
(function (context, factory) {
    ...
    flipPlacement: function (step) {
      if (step.isRtl && !step._isFlipped) {
        ...
        for (i in props) {
          prop = props[i];
          if (
            step.hasOwnProperty(prop) &&
            rtlMatches.hasOwnProperty(step[prop])
          ) {
  ...
  HopscotchBubble.prototype = {
    ...
    setPosition: function (step) {
      ...
      verticalLeftPosition = step.isRtl
        ? boundingRect.right - bubbleBoundingWidth
        : boundingRect.left;
      ...
      if (step.placement === "top") {
        ...
        throw new Error(
          "Bubble placement failed because step.placement is invalid or undefined!"
        );
      }
      ...
      if (!arrowOffset) {
        ...
        if (arrowOffset === "center") {
          arrowEl.style[arrowPos] =
            Math.floor(bubbleBoundingWidth / 2 - arrowEl.offsetWidth / 2) +
            "px";
        } else {
          // Numeric pixel value
        ...
        if (arrowOffset === "center") {
          arrowEl.style.top =
            Math.floor(bubbleBoundingHeight / 2 - arrowEl.offsetHeight / 2) +
            "px";
        } else {
          // Numeric pixel value
    ...
    render: function (step, idx, callback) {
      ...
      if (this.opt.isTourBubble) {
        ...
        if (currTour) {
          ...
          step.isRtl = step.hasOwnProperty("isRtl")
            ? step.isRtl
            : currTour.hasOwnProperty("isRtl")
            ? currTour.isRtl
            : this.opt.isRtl;
          unsafe = currTour.unsafe;
          if (Array.isArray(currTour.steps)) {
            totalSteps = currTour.steps.length;
            totalStepsI18n = this._getStepI18nNum(
              this._getStepNum(totalSteps - 1)
            );
      ...
      opts = {
        ...
        buttons: {
          showPrev:
            utils.valOrDefault(step.showPrevButton, this.opt.showPrevButton) &&
            this._getStepNum(idx) > 0,
          showNext: utils.valOrDefault(
            step.showNextButton,
            this.opt.showNextButton
          ),
          showCTA: utils.valOrDefault(
            step.showCTAButton && step.ctaLabel,
            false
          ),
      ...
      if (typeof tourSpecificRenderer === "function") {
        ...
        if (
          !winHopscotch.templates ||
          typeof winHopscotch.templates[tourSpecificRenderer] !== "function"
        ) {
          throw new Error(
            'Bubble rendering failed - template "' +
              tourSpecificRenderer +
              '" is not a function.'
          );
        }
        ...
        if (
          !winHopscotch.templates ||
          typeof winHopscotch.templates[templateToUse] !== "function"
        ) {
          throw new Error(
            'Bubble rendering failed - template "' +
              templateToUse +
    ...
    hide: function (remove) {
      ...
      utils.removeClass(
        el,
        "animate fade-in-up fade-in-down fade-in-right fade-in-left"
      );
  ...
  HopscotchCalloutManager = function () {
    ...
    this.createCallout = function (opt) {
      ...
      if (opt.id) {
        ...
        if (callouts[opt.id]) {
          throw new Error(
            "Callout by that id already exists. Please choose a unique id."
          );
        }
        if (!utils.getStepTarget(opt)) {
          throw new Error(
            "Must specify existing target element via 'target' option."
          );
        }
    ...
    this.refreshCalloutPositions = function () {
      ...
      for (calloutId in callouts) {
        if (
          callouts.hasOwnProperty(calloutId) &&
          calloutOpts.hasOwnProperty(calloutId)
        ) {
  ...
  Hopscotch = function (initOptions) {
    var self = this, // for targetClickNextFn
      ...
      getCurrStep = function () {
        ...
        if (
          !currTour ||
          currStepNum < 0 ||
          currStepNum >= currTour.steps.length
        ) {
          step = null;
        } else {
      ...
      adjustWindowScroll = function (cb) {
        var bubble = getBubble(),
          ...
          targetBottom =
            bubbleBottom > targetElBottom ? bubbleBottom : targetElBottom,
        ...
        if (
          targetTop >= windowTop &&
          (targetTop <= windowTop + getOption("scrollTopMargin") ||
            targetBottom <= windowBottom)
        ) {
          if (cb) {
            cb();
        ...
        // Smooth scroll to scroll target
          ...
          if (
            ...
            scrollEl = YAHOO.env.ua.webkit
              ? document.body
              : document.documentElement;
            yuiEase = YAHOO.util.Easing ? YAHOO.util.Easing.easeOut : undefined;
            yuiAnim = new YAHOO.util.Scroll(
              scrollEl,
              {
                scroll: { to: [0, scrollToVal] },
              },
              getOption("scrollDuration") / 1000,
              yuiEase
            );
          ...
          // Use my crummy setInterval scroll solution if we're using plain, vanilla Javascript.
            ...
            scrollIncr =
              Math.abs(windowTop - scrollToVal) /
              (getOption("scrollDuration") / 10);
      ...
      changeStep = function (doCallbacks, direction) {
        ...
        if (step.nextOnTargetClick) {
          ...
          utils.removeEvtListener(
            utils.getStepTarget(step),
            "click",
            targetClickNextFn
          );
        }
        ...
        if (direction > 0) {
          ...
          wasMultiPage =
            currStepNum > 0 && currTour.steps[currStepNum - 1].multipage;
        }
        ...
        changeStepCb = function (stepNum) {
          ...
          if (doCallbacks) {
            if (direction > 0) {
              doShowFollowingStep = utils.invokeEventCallbacks(
                "next",
                origStep.onNext
              );
            } else {
      ...
      showStepHelper = function (stepNum) {
        ...
        if (currStepNum !== stepNum && getCurrStep().nextOnTargetClick) {
          // Detach the listener when tour is moving to a different step
          utils.removeEvtListener(
            utils.getStepTarget(getCurrStep()),
            "click",
            targetClickNextFn
          );
        }
    ...
    this.startTour = function (tour, stepNum) {
      ...
      if (
        typeof currStepNum === "undefined" &&
        currTour.id === cookieTourId &&
        typeof cookieTourStep !== undefinedStr
      ) {
      ...
      findStartingStep(currStepNum, skippedSteps, function (stepNum) {
        var target =
          stepNum !== -1 && utils.getStepTarget(currTour.steps[stepNum]);
    ...
    this.endTour = function (clearState, doCallbacks) {
      ...
      if (currTour) {
        ...
        if (currentStep && currentStep.nextOnTargetClick) {
          utils.removeEvtListener(
            utils.getStepTarget(currentStep),
            "click",
            targetClickNextFn
          );
        }
    ...
    _configure = function (options, isTourOptions) {
      ...
      for (i = 0, len = events.length; i < len; ++i) {
        ...
        // to a function.
        eventPropName =
          "on" + events[i].charAt(0).toUpperCase() + events[i].substring(1);
    ...
    steps: [
      {
        ...
        title: "Welcome to Bootstrap grid examples!",
        content:
          "Hey there! This is where we start our website tour. It's gonna be awesome.",
        placement: "bottom",
        arrowOffset: 60,
      },
      {
        target: "general-use-desc",
        title: "This is pointing to a specific word",
        content:
          "Just to show this plugin is flexible enough to target even words",
        placement: "right",
        yOffset: -20,
      },
      {
        ...
        placement: "top",
        title: "Grid for mobile and desktop?",
        content:
          "In order to input a question start by choosing atemplate by clicking on 'Choose Template' button. (Screen to gif 'choose your template button').Input numbers into spaces provided.",
      },
      {
        target: "start-tour",
        placement: "right",
        ...
        content:
          "After you've created your tour, pass it in to the startTour() method to start it.",
        yOffset: -25,
      },
      {
        ...
        content:
          "These are the most basic step options: <b>target</b>, <b>title</b>, <b>content</b>, and <b>placement</b>. For some steps, they may be all you need.",
        arrowOffset: 100,
        yOffset: -80,
      },
      {
    ...
    if (state && state.indexOf("hello-hopscotch:") === 0) {
      ...
      setTimeout(function () {
        mgr.createCallout({
          ...
          content:
            "Start by taking an example tour to see Hopscotch in action!",
          yOffset: -25,
          arrowOffset: 20,
          width: 240,
        });
